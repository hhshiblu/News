...
import { createAdminUserAction, patchAdminUserStatusAction } from "@/actions/admin-data.action";
...
function RoleBadge({ role }) {
  ...
  const LABELS = {
    ...
    AUTHOR: "Author",
    REPORTER: "Reporter",
    RESEARCH_AUTHOR: "Research author",
  };
  const label = LABELS[k] || (k ? String(k).replace(/_/g, " ") : "—");
  const cls = ROLE_STYLE[k] || "bg-gray-50 border-gray-200 text-gray-700";
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-md border font-semibold text-[10px] tracking-wide ${cls}`}>{label}</span>
  );
}
...
export default function AuthorsManagementClient({ initialAuthors = [], statusFilter = "ALL" }) {
  ...
  const uploadAvatar = async (file) => {
    ...
    if (!res.ok || !data.url) throw new Error(data.message || "Avatar upload failed");
    return data.url;
  };
  ...
  const submitNewAuthor = async (e) => {
    ...
    try {
      ...
      const socials = {};
      if (newAuthor.twitter.trim()) socials.twitter = newAuthor.twitter.trim();
      if (newAuthor.linkedin.trim()) socials.linkedin = newAuthor.linkedin.trim();
      if (newAuthor.website.trim()) socials.website = newAuthor.website.trim();
      ...
      const payload = {
        ...
        role: newAuthor.role,
        avatar,
        position: newAuthor.position.trim() || undefined,
      ...
      if (!res.success) throw new Error(res.message || "Create author failed");
          ...
          <div className="bg-white rounded-t-2xl sm:rounded-xl shadow-xl w-full sm:max-w-md max-h-[92vh] sm:max-h-[90vh] overflow-hidden border border-gray-200 flex flex-col animate-in slide-in-from-bottom-4 duration-200">
            ...
            <div className="p-4 sm:p-5 space-y-4 overflow-y-auto flex-1">
              ...
              {selectedAuthor.position ? (
                <div>
                  <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1.5">Role title</p>
                  <p className="text-sm font-semibold text-gray-800">{selectedAuthor.position}</p>
                </div>
              ) : null}
              <div>
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1.5">Joined</p>
                <p className="text-sm font-medium text-gray-700">
                  {selectedAuthor.createdAt ? new Date(selectedAuthor.createdAt).toLocaleString() : "—"}
                </p>
              </div>
              <div>
              ...
              typeof selectedAuthor.socials === "object" &&
              Object.values(selectedAuthor.socials).some(Boolean) ? (
                <div>
                  <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2">Social</p>
                  <ul className="space-y-1 text-[12px]">
                    {Object.entries(selectedAuthor.socials).map(([key, val]) =>
                      val ? (
                        <li key={key}>
                          <span className="font-semibold text-gray-600">{key}: </span>
                          <span className="break-all text-gray-800">{String(val)}</span>
                        </li>
                      ) : null
                    )}
                  </ul>
          ...
          <div className="w-full sm:max-w-2xl max-h-[94vh] overflow-y-auto rounded-t-2xl sm:rounded-2xl border border-gray-200 bg-white shadow-2xl">
            <div className="sticky top-0 z-10 flex items-center justify-between border-b border-gray-100 bg-white/95 px-4 py-3 backdrop-blur">
              <h3 className="text-sm font-extrabold text-gray-900 flex items-center gap-2">
                <UserPlus className="h-4 w-4 text-primary" /> Add New Author
              </h3>
              <button
                type="button"
            ...
            <form onSubmit={submitNewAuthor} className="space-y-4 px-4 py-4 sm:px-5 sm:py-5">
              <div className="rounded-2xl border border-gray-200 bg-gray-50/70 p-3">
                <p className="mb-2 text-[10px] font-black uppercase tracking-widest text-gray-500">Author image (required)</p>
                <div className="flex items-center gap-3">
                  <button
                    type="button"
                  ...
                  <div className="text-[12px] text-gray-600">
                    <p className="font-semibold text-gray-800">Click the box to upload avatar</p>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <label className="sm:col-span-2">
                  <span className="mb-1 block text-[10px] font-black uppercase tracking-widest text-gray-500">Full name</span>
                  <input
                    required
                    value={newAuthor.name}
                    onChange={(e) => setNewAuthor((s) => ({ ...s, name: e.target.value }))}
                    className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm"
                  />
                </label>
                <label>
                  <span className="mb-1 block text-[10px] font-black uppercase tracking-widest text-gray-500">Email</span>
                  <input
                    required
                    type="email"
                    value={newAuthor.email}
                    onChange={(e) => setNewAuthor((s) => ({ ...s, email: e.target.value }))}
                    className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm"
                  />
                </label>
                <label>
                  <span className="mb-1 block text-[10px] font-black uppercase tracking-widest text-gray-500">Password</span>
                  <input
                    required
                    type="password"
                    value={newAuthor.password}
                    onChange={(e) => setNewAuthor((s) => ({ ...s, password: e.target.value }))}
                    className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm"
                  />
                </label>
                <label>
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewAuthor((s) => ({ ...s, position: e.target.value }))}
                    className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm"
                  />
                </label>
                <label className="sm:col-span-2">
                  <span className="mb-1 block text-[10px] font-black uppercase tracking-widest text-gray-500">Bio</span>
                  <textarea
                    rows={3}
                    value={newAuthor.bio}
                    onChange={(e) => setNewAuthor((s) => ({ ...s, bio: e.target.value }))}
                    className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm"
                  />
                </label>
                <label>
                  <span className="mb-1 block text-[10px] font-black uppercase tracking-widest text-gray-500">Twitter</span>
                  <input
                    value={newAuthor.twitter}
                    onChange={(e) => setNewAuthor((s) => ({ ...s, twitter: e.target.value }))}
                    className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm"
                  />
                </label>
                <label>
                  <span className="mb-1 block text-[10px] font-black uppercase tracking-widest text-gray-500">LinkedIn</span>
                  <input
                    value={newAuthor.linkedin}
                    onChange={(e) => setNewAuthor((s) => ({ ...s, linkedin: e.target.value }))}
                    className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm"
                  />
                </label>
                <label className="sm:col-span-2">
                  <span className="mb-1 block text-[10px] font-black uppercase tracking-widest text-gray-500">Website</span>
                  <input
                    value={newAuthor.website}
                    onChange={(e) => setNewAuthor((s) => ({ ...s, website: e.target.value }))}
                    className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm"
                  />
                </label>
              </div>

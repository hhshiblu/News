...
export default async function StoriesAdminPage({ searchParams }) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-700 pb-20 px-1 pt-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Manage and publish narrative stories across the platform.
          </p>
        </div>
        <Link
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <div className="space-y-1">
                      <p className="font-bold text-gray-900 line-clamp-1 group-hover:text-primary transition-colors">
                        {story.title}
                      </p>
                  ...
                  <td className="px-6 py-4 text-center">
                    <span
                      className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${story.status === "ACTIVE" ? "bg-emerald-50 text-emerald-700 border-emerald-100" : "bg-orange-50 text-orange-600 border-orange-100"}`}
                    >
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${story.status === "ACTIVE" ? "bg-emerald-500" : "bg-orange-500 animate-pulse"}`}
                      />
                      {story.status}
                    </span>
                  </td>
                  ...
                  <td colSpan={4} className="px-6 py-20 text-center">
                    <div className="max-w-xs mx-auto space-y-2">
                      ...
                      <p className="text-xs text-gray-400">
                        Get started by creating your first story narrative.
                      </p>
                    </div>
                  </td>
                </tr>

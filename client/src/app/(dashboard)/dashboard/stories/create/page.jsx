...
import { Type, Trash2, Upload, MousePointer2, Image as ImageIcon, X, SquarePen } from "lucide-react";
...
export default function CreateStoryPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!title || !thumbnailImageFile) {
        toast.error("Please provide a title and a thumbnail image!");
        return;
    }
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4 bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
          <div className="flex items-center gap-4">
             ...
             <div>
                <h1 className="text-xl font-black text-gray-900 tracking-tight">Create New Story</h1>
                <p className="text-xs text-gray-500 font-medium">Compose a narrative-driven story</p>
             </div>
          </div>
          <button onClick={handleSubmit} className="px-8 py-3 bg-primary text-white font-black text-xs rounded-2xl hover:bg-primary-dark transition-all shadow-lg shadow-primary/20 uppercase tracking-[0.2em] flex items-center justify-center gap-2 cursor-pointer active:scale-95">
            <MousePointer2 size={16} /> Publish Story
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-6">
               ...
               <p className="text-[10px] text-gray-400 font-bold text-center uppercase tracking-tighter italic">Recommended size: 800x800px</p>
            </div>

            <div className="bg-gray-900 p-6 rounded-3xl shadow-xl text-white space-y-4">
               <h3 className="text-xs font-black uppercase tracking-[0.2em] flex items-center gap-2">
                  <span className="w-2 h-2 bg-primary rounded-full animate-pulse" /> Tips
               </h3>
               <ul className="space-y-3">
                  <li className="text-[11px] text-gray-400 leading-relaxed">• Keep titles punchy and evocative.</li>
                  <li className="text-[11px] text-gray-400 leading-relaxed">• Use high-quality visual thumbnails.</li>
                  <li className="text-[11px] text-gray-400 leading-relaxed">• Break up text with relevant images.</li>
               </ul>
            </div>
          </div>
        </div>

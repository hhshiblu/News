...
import { Check, X, Shield, Users, Mail, RefreshCw, Edit, Eye, Activity, AlertCircle, Clock, FileText, ExternalLink } from "lucide-react";
import { toast } from "sonner";
import { useRouter, usePathname, useSearchParams } from 'next/navigation';
import Link from "next/link";

export default function AuthorsClient({ initialAuthors, filterParam }) {
  ...
  const getRoleBadge = (role) => {
      switch(role) {
          case 'AUTHOR': return <span className="px-2 py-0.5 rounded bg-purple-50 border border-purple-200 text-purple-700 font-semibold text-[10px] tracking-wide">AUTHOR</span>;
                ...
                <tr key={author.id} className="hover:bg-gray-50/70 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1.5 text-xs font-medium">
                        <Link href={`/dashboard/posts?authorId=${author.id}`} className="px-2 py-1 bg-purple-50 text-purple-700 border border-purple-200 rounded hover:bg-purple-100 transition-colors flex items-center gap-1.5 cursor-pointer" title="View All News">
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                       ...
                       <p className="text-sm font-bold text-gray-900 flex items-center gap-1.5">
                           ...
                           {selectedAuthor.posts ? selectedAuthor.posts.length : 0}
                       </p>
                    </div>
                    <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                       <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest mb-1">Active Posts</p>
                       <p className="text-sm font-bold text-gray-900 flex items-center gap-1.5">
                           <Activity className="w-3.5 h-3.5 text-emerald-500" />
                           {selectedAuthor.posts ? selectedAuthor.posts.filter(p => p.status === 'PUBLISHED').length : 0}
                       </p>
                    </div>
                </div>

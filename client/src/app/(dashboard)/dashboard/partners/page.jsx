        ...
        <form onSubmit={onSubmit} className="space-y-5 rounded-2xl border border-gray-200 bg-white p-4 md:p-6 shadow-sm">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <FormField label="Priority" htmlFor="pt-pri" hint="Higher appears first in sliders">
              <input id="pt-pri" type="number" className={numCls} value={form.priority} onChange={(e) => setForm({ ...form, priority: Number(e.target.value) || 0 })} />
            </FormField>
          </div>
          ...
          <FormField label="Logo" hint={editingId ? "Optional — click image to replace; max 1 file" : "Required — max 1 image"}>
            <ImageUploadPreview
              file={imageFile}
                  ...
                  <tr key={it.id} className="border-t text-[13px]">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <img src={it.logoUrl} alt={it.name} className="h-10 w-14 rounded-md border bg-gray-50 object-contain p-1" />
                        <div>

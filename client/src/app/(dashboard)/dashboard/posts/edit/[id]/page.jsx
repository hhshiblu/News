...
import { Type, Video, Trash2, Upload, Tag, Quote, Activity, Shield, X, ChevronDown, MousePointer2, FileText, SquarePen, Image as ImageIcon, Star, Zap, Save } from "lucide-react";
...
export default function EditPostPage({ params }) {
     ...
     Promise.all([fetchCategories, fetchTags, fetchPost, fetchMe]).then(([catsData, tagsData, postsData, meData]) => {
         ...
         if(postsData.success) {
            ...
            if(target) {
                ...
                setHeroCaption(target.heroCaption || "");
                setCountryTag(target.countryTag || "");
                setFeatured(!!target.featured);
                setIsOpinion(!!target.isOpinion);
                setHomeSpotlightPriority(typeof target.homeSpotlightPriority === "number" ? target.homeSpotlightPriority : 0);
                setFeedPriority(typeof target.feedPriority === "number" ? target.feedPriority : 0);
                
                if (target.featuredImage) {
                    setFeaturedImagePreview(target.featuredImage);
                }

                let pId = target.categoryId;
                let cId = "";
                if (catsData.success && catsData.data) {
                    for (const cat of catsData.data) {
                        if (cat.id === target.categoryId) {
                            pId = cat.id; break;
                        }
                        if (cat.children && cat.children.some(ch => ch.id === target.categoryId)) {
                            pId = cat.id;
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    if (!title || !parentCategoryId || selectedTags.length === 0) {
        toast.error("Please fill in all mandatory fields (Title, Category, Tags)!");
        return;
    }
    ...
    try {
        let uploadedFeaturedImage = featuredImagePreview;
  ...
  if (isLoading) return <div className="p-10 text-center text-[12px] font-semibold text-gray-500 animate-pulse">Loading Editor...</div>;
    ...
    <div className="w-full px-1 md:px-4 pb-28 animate-in fade-in slide-in-from-bottom-4 duration-700 bg-[#f8fafc] min-h-screen">
      <div className="max-w-[1400px] mx-auto pt-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4 bg-white p-5 md:p-6 rounded-2xl border border-gray-100 shadow-sm">
          <div>
            <h1 className="text-[14px] font-black text-gray-900 tracking-tight uppercase">Edit Article</h1>
            <p className="text-[12px] text-gray-500 mt-1 font-medium tracking-wide">Premium Editorial Suite</p>
          </div>
          <button onClick={handleUpdateSubmit} className="px-6 py-2.5 bg-[#8B0000] text-white font-bold text-[12px] rounded-lg hover:bg-[#6b0000] transition-colors shadow-sm uppercase tracking-wider flex items-center justify-center gap-2 cursor-pointer">
        ...
        <div className="flex flex-col-reverse lg:grid lg:grid-cols-12 gap-4 md:gap-6">
              ...
              <div className="bg-white p-2 md:p-8 rounded-2xl border border-gray-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] space-y-8">
                  <div className="relative">
                     ...
                     <input 
                       ...
                       maxLength={MAX_TITLE}
                       className="w-full bg-transparent border-0 border-b-2 border-gray-100 px-1 py-2 text-[14px] font-bold focus:ring-0 focus:border-[#8B0000] placeholder:text-gray-300 text-gray-900 transition-colors outline-none" 
          ...
          <div className={`${isAdmin ? 'lg:col-span-12 max-w-2xl mx-auto w-full' : 'lg:col-span-4'} space-y-4 md:space-y-6`}>
              ...
              <div className="bg-white rounded-2xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-gray-100 p-4 md:p-5 space-y-4">
                  <div className="flex items-center justify-between">
                      ...
                      {featuredImagePreview && (
                          <button onClick={() => { setFeaturedImagePreview(""); setFeaturedImageFile(null); }} className="text-[12px] font-bold text-rose-600 bg-rose-50 px-2 py-0.5 rounded-md hover:bg-rose-100 transition-colors cursor-pointer">
                              Remove
                          </button>
                      )}
                  </div>
                  ...
                  {featuredImagePreview ? (
                      <div className="relative group rounded-lg overflow-hidden border border-gray-200 aspect-[16/10] shadow-sm">
                          <img src={featuredImagePreview} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" alt="Featured Preview" />
                          <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                               <div onClick={() => { setFeaturedImagePreview(""); setFeaturedImageFile(null); }} className="w-10 h-10 bg-white rounded-full flex items-center justify-center cursor-pointer hover:bg-rose-600 hover:text-white text-gray-800 shadow-sm transition-all">
                                  <Trash2 size={16} />
                               </div>
                          </div>
                      </div>
